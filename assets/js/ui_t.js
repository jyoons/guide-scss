/** ---------------------------------------------------------------
 ...
 HWSB.select (설렉트타입)
 HWSB.accordion (아코디언메뉴)
 HWSB.fixBody (화면고정용)
 HWSB.alertPop //open(웹접근성용버튼대상,data-id,메세지),close(data-id)
 HWSB.confirmPop //open(웹접근성용버튼대상,data-id),close(data-id)
 HWSB.modalLayer //open(웹접근성용버튼대상,data-id),close(data-id)
 HWSB.layerFull //open(웹접근성용버튼대상,data-id),close(data-id)
 HWSB.onlyDimmed 사용하지않음
 HWSB.loading //open(),close()
 HWSB.toast(웹접근성용버튼대상,메세지)
     ...
     HWSB = {
         ...
         layerFull: {
             open: function(ele,unique) {
                 ...
                 $(ele).attr("data-focus", unique);
                 let uniqueId = $('[data-id=' + unique + ']');
                 //👋2022.06.09 웹접근성관련 수정
                 uniqueId
                     .addClass("active")
                     .css({"z-index":++popIndex})
                     .attr("tabindex", "0")
                     .focus()
                     .append(focusMove);
                 $(".pop-loop").on("focus", function() {
             ...
             close: function(unique) {
                 let uniqueId = $('[data-id=' + unique + ']'),
                     focusEle = $('[data-focus=' + unique + ']'),
                     lyrLength = $(".lyr-popup-wrap.type-full:visible");
                 //👋2022.06.09 웹접근성관련 수정
                 uniqueId
                     .removeClass("active")
                     .attr("style","")
                     .find('a.pop-loop, div.dimmed')
                     .remove();
                 // //👋2022.06.09 웹접근성관련 수정
                 if (lyrLength.length == 1) {
                     HWSB.fixBody.disable();
         ...
         scrollTar:function(tar) {
             const $tar = $(tar);
             if(!$tar.hasClass("range")){
                 var tarH =($tar.offset().top - 90) + $(".wrap").scrollTop(); //높이여백 수치값 변경
         ...
         activeScroll:function(tarTop){
             let activeHeight = $(tarTop).height() + $("[data-role='header']").height() - 250;
             let event01 = activeHeight;
             let event02 = event01 + $(".visual-item.-type01").outerHeight();
             let event03 = event02 + $(".visual-item.-type02").outerHeight()*1.2;
             let event04 = event03 + $(".visual-item.-type03").outerHeight();
             $("[data-role='wrap']").scroll(function(){
 ...
 $(document).ready(function(){
     ...
     if(tabGroups){
         ...
         //푸터네비 고정
         if($("body").find('.btn-appbar').length){
     ...
     $("input,textarea").on("focusin",function(){
         if(!$(this).hasClass("range")){
             var tarH =($(this).offset().top - 90) + $(".wrap").scrollTop(); //높이여백 수치값 변경
             $(".wrap").stop().animate({scrollTop:tarH},500,function(){
                 $(this).focus();
             });
         }
     ...
     $(".has-etc input[type=radio]").change(function (){
         const $this = $(this);
         if($this.hasClass("has-ipt") && $this.is(":checked")){
             $this.closest(".has-etc").find(".hidden-ipt").show().find(".ipt").focus();
         }else{
             $this.closest(".has-etc").find(".hidden-ipt").hide();
         }
     });
     //터치 이벤트 색상 테스트
